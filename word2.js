...
function calculateTotalWords() {
  return selectdayList.length * 10;
}

function getrandomNum(event) {
  // 로컬 스토리지에서 선택된 날짜 복원
  const storedDays = localStorage.getItem("selectedDay");
  if (storedDays) {
    selectdayList = JSON.parse(storedDays); // 복원된 날짜 리스트
  }

  const testNum = event.target.innerText;
  const regex = testNum.replace(/[^0-9]/g, "");
  randomTestNum = parseInt(regex, 10); // 숫자로 변환하여 저장

  const totalWords = calculateTotalWords(); // 총 단어 수 계산
  console.log(totalWords);

  if(totalWords< randomTestNum){
    alert("선택된 테스트의 단어의 수가 적습니다. ");
    return;
  }
